    ...
    <q-page-container>
      <q-page padding>
        ...
        <q-list bordered separator>
          <draggable v-model="todos" item-key="id" handle=".drag-handle" @end="onDragEnd">
            <template #item="{ element: todo }">
              <q-item>
                ...
                <q-item-section avatar>
                  <q-checkbox v-model="todo.completed" @update:model-value="toggleTodo(todo)" />
                </q-item-section>
                ...
                <q-item-section side>
                  <q-btn flat round dense color="negative" @click="deleteTodo(todo.id)">
                    <template v-slot:default>
...
const addTodo = async () => {
  ...
  try {
    ...
    showNotification(
      `Failed to add todo: ${error.response?.data?.detail || 'An error occurred while adding the todo'}`,
      'negative'
    );
  }
};
...
const toggleTodo = async (todo) => {
  try {
    ...
    showNotification(
      ...
      todo.completed ? 'positive' : 'info'
    );
  } catch (error) {
    console.error('Error updating todo:', error);
    showNotification(
      `Failed to update todo: ${error.response?.data?.detail || 'An error occurred while updating the todo'}`,
      'negative'
    );
  }
};
...
const deleteTodo = async (id) => {
  try {
    ...
    showNotification(
      `Failed to delete todo: ${error.response?.data?.detail || 'An error occurred while deleting the todo'}`,
      'negative'
    );
  }
};
