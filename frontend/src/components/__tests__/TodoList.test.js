...
vi.mock('quasar', () => ({
  ...
  QInput: {
    name: 'QInput',
    template: '<div class="q-input todo-input"><input :value="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" @keyup.enter="$emit(\'keyup.enter\')" /><slot name="append" /></div>',
    props: ['modelValue'],
  ...
  QCheckbox: {
    name: 'QCheckbox',
    template: '<div class="q-checkbox todo-checkbox"><input type="checkbox" :checked="modelValue" @change="$emit(\'update:modelValue\', $event.target.checked)" /></div>',
    props: ['modelValue'],
    emits: ['update:modelValue']
  },
  QBtn: {
    name: 'QBtn',
    template: '<button class="q-btn" :class="$attrs.class" @click="$emit(\'click\')"><slot /></button>',
